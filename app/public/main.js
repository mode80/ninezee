...
function Jahtzee() {
  ...
  proto.calcVal = function(die_array) {
    if(die_array.allSame() && this.player.yahtzee.val > 0) return 100;
    else return 0
  }
  proto.proposeVal = function(die_array) {
    this.val += this.calcVal(die_array)
  ...
  function Player(name, game) {
    ...
    this.lower_scores.push(this.three_of_a_kind, this.four_of_a_kind, this.full_house, this.sm_straight, this.lg_straight, this.chance)
  ...
  var Dice = function() {
      ...
      dice.rollSelected = function() {
        var selectedDice = this.filter(function(die) {
          return die.selected;
        })
        selectedDice.each(function(die) {
          die.roll()
        })
      }

      dice.selectAll = function() {
        dice.each(function(die) {
          die.selected = true
        })
      }
      ...
      dice.sumOfDice = function() {
        return this.reduce(function(sum, die) {
          return sum + die.val
        }, 0)
      }
  ...
  proto.newPlayer = function(playerTypeString) {
    ...
    var player_name = playerTypeString ? playerTypeString + (Math.floor(Math.random() * 900) + 100) : "Player " + (this.players.length + 1)
